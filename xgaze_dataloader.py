...
def get_train_loader(data_dir,
    ...
    train_set = GazeDataset(dataset_path=data_dir, keys_to_use=datastore[sub_folder_use], sub_folder=sub_folder_use,
                            transform=trans, is_shuffle=is_shuffle, is_load_label=True, subject=subject, key_data= key_data)
...
def get_val_loader(data_dir,
    ...
    val_set = GazeDataset(dataset_path=data_dir, keys_to_use=datastore[sub_folder_use], sub_folder=sub_folder_use,
                            transform=trans, is_shuffle=is_shuffle, is_load_label=True,subject=subject, num_val_images = num_val_images, key_data= key_data)
...
class GazeDataset(Dataset):
    def __init__(self, dataset_path: str, keys_to_use: List[str] = None, sub_folder='', transform=None, is_shuffle=True,
        ...
        self.n = n
        self.target_idx = np.loadtxt("xgaze_evaluation_target_single_subject.txt", dtype=np.int)
    ...
    def __getitem__(self, idx):
        ...
        if self.is_load_label:
            ...
            entry = {
            self.key_data + '_key': key,
            self.key_data + '_image': image,
            self.key_data + '_gaze': gaze_label,
            self.key_data + '_head': head_label,
        }
